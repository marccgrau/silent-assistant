...
3. **Add secrets**:
   ...
   ```bash
   cp .env.example .env
   # Edit .env and add your API keys (Deepgram, OpenAI, Supabase service role, etc.)
   ```

4. **Configure non-secret settings and prompt**:

   ```bash
   cp config.example.toml config.toml
   # Edit config.toml for Deepgram options, advisor model, Supabase URL/bucket, participant hints, etc.
   # Edit prompts/advisor_system_prompt.txt if you want a custom advisor prompt.
   ```
